...
void initlistensocket(int ep_fd, unsigned short port) {
	...
	/*绑定*/
	bind(listen_fd, (struct sockaddr *) &listen_socket_addr,
			sizeof(listen_socket_addr));
	/*设置监听上限*/
	listen(listen_fd, 128);

	/*将listen_fd初始化*/
	eventset(&ep_events[MAX_EVENTS - 1], listen_fd, acceptconnect,
			&ep_events[MAX_EVENTS - 1]);
	/*将listen_fd挂上红黑树*/
...
/*将结构体成员变量初始化*/
void eventset(struct my_events *my_ev, int fd,
		void (*call_back)(int, int, void *), void *event_arg) {
	my_ev->m_fd = fd;
...
void acceptconnect(int listen_fd, int event, void *arg) {
	...
	if ((connect_fd = accept(listen_fd,
			(struct sockaddr *) &connect_socket_addr, &connect_socket_len))
			< 0) {
		if (errno != EAGAIN && errno != EINTR) {/*暂时不处理*/
		}
		printf("\n %s: accept, %s \n", __func__, strerror(errno));
		return;
	}
